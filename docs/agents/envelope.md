...
| `outside_pack_scope` | запит вийшов за межі, які пак сам про себе декларує: розділ, воєводство, конфесія, повіт, якого газетир не знає | нуль тут про межу довідника — назвати її й узяти канал, що покриває регіон |
| `source_undeclared` | джерело (плагін) не має самоопису: невідомо, де його межі й що доводить його нуль | ставити в план рядком «без опису — перевірити руками» |

У `catalog.search` повний нуль приходить із `zero_with_denominator`, у тексті
якого стоїть, що доводить нуль кожного джерела; те саме структуровано лежить у
`data.coverage.zeros`, а на чому джерело шукає збіг — у `coverage.basis`
(`match`, `match_how`). Типи нуля — не шукали, поза покриттям, шукали й немає,
обрізано, хибний не-нуль, однойменець — розібрані в `where-to-dig.md`,
розділ «Типи нуля».
