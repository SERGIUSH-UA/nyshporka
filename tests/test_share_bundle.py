...
def test_geometry_is_opt_in(space: Path, tmp_path: Path) -> None:
    """Геометрія важить ×10 і без кадрів отримувачу марна — тільки на вимогу."""
    run = make_run(space / "reports" / "htr", "spr-8433", geometry=True)
    m = manifest_for([bundle.voice_of(run, geometry=False)])

    plain = tmp_path / "plain.nyshtext"
    bundle.write(plain, m, [run], geometry=False)
    with tarfile.open(plain) as tar:
        assert not [n for n in tar.getnames() if n.endswith(".lines.json")]

    rich = tmp_path / "rich.nyshtext"
    bundle.write(rich, m, [run], geometry=True)
    with tarfile.open(rich) as tar:
...
def test_voices_travel_as_sibling_dirs(space: Path, tmp_path: Path) -> None:
    ...
    m = manifest_for([bundle.voice_of(main, geometry=False),
                      bundle.voice_of(voice, geometry=False)])
    dest = tmp_path / "pack.nyshtext"
...
def test_readme_carries_the_denominator(space: Path, tmp_path: Path) -> None:
    ...
    run = make_run(space / "reports" / "htr", "spr-8433")
    m = manifest_for([bundle.voice_of(run, geometry=False)], pages=3, frames=10)
