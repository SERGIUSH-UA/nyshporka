...
не виставляють) — за спільним контрактом джерела: `search` / `browse` /
`manifest` / `fetch`. Найцінніше в
...
говірок. Пошук іде по **повному тексту** книг, а не лише по назві, але уривка
сервер не дає — знахідка каже «слово є в цій книзі», а сторінку показує
текстовий шар, що лягає поруч із завантаженим PDF (`<файл>.pdf.text/p0012.txt`).
