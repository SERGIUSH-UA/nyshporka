...
> **Стан: alpha.** Каталоги, завантаження, сховище прочитаного, пошук, браузерне
> обличчя й установлення працюють. Ваги моделей ще не викладені релізом, тож
> читання рукопису поки доступне лише з власними вагами. Що саме не готове —
> у розділі «Чого ще немає» нижче, без замовчувань.
...
* **Ваги моделей не викладені.** `nysh models get` знає, що качати, але реліз із
  ...
  відрізняється від поганого почерку.
* **Читання рукопису запускається з командного рядка, не з застосунку.** Раннер
  працює й перевірений, але постановка прогону в чергу з браузера ще не зроблена.
* **Гортача сторінок немає.** Перегляд сканів із рамками рядків — наступний крок.
