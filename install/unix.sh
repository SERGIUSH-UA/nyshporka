...
# ── Mac з процесором Intel: рушії читання сюди не стануть ───────────────────
# 🔴 Не «цього разу», а ніколи (issue #10): PyTorch не випускає колес для
# macOS x86_64 з torch 2.2.2, а `kraken==7.0.2` — пін під патчі — вимагає
# torch ≥ 2.4. Без цієї гілки резолвер падав на `torchvision … macosx_13_0_x86_64`
# ПІСЛЯ того, як uv і Python уже стали, і людина читала це як свою помилку.
# Тому extra `htr` знімається до установки, а на його місце йде `cloud`:
# читання на іншій машині (`nysh cloud`) — єдиний спосіб читати з такого Mac,
# і без `paramiko` воно не працює.
# ⚠ `uname -m` під Rosetta теж каже x86_64 — і колеса тоді потрібні ті самі.
# ⚠ Явний NYSH_SOURCE не чіпається: хто задав склад сам, той знає, що робить.
MAC_INTEL=0
if [ -z "${NYSH_SOURCE:-}" ] && [ "$(uname -s 2>/dev/null)" = Darwin ] \
   && [ "$(uname -m 2>/dev/null)" = x86_64 ]; then
  case "$SOURCE" in
    *,htr*)
      MAC_INTEL=1
      SOURCE=$(printf '%s' "$SOURCE" | sed 's/,htr/,cloud/')
      say "⚠ Mac з процесором Intel: рушії читання (torch) тут не ставляться —"
      say "  PyTorch не випускає колес для macOS x86_64 з версії 2.2.2, а рушіям"
      say "  потрібна 2.4 або новіша. Ставлю $SOURCE:"
      say "  усе, крім читання на цій машині; читати — на іншій, через nysh cloud."
      say ""
      ;;
  esac
fi
