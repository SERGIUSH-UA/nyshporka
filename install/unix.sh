...
# ── Mac з процесором Intel: рушії читання збираються інакше ─────────────────
# 🔴 Issue #10: PyTorch не випускає колес для macOS x86_64 з torch 2.2.2, а
# `kraken==7.0.2` — пін під патчі — вимагає torch ≥ 2.4. Доти резолвер падав
# на `torchvision … macosx_13_0_x86_64` ПІСЛЯ того, як uv і Python уже стали,
# і людина читала це як свою помилку. Тепер extra `htr` у пакеті на такій
# машині порожній (маркер платформи в pyproject), тож установка проходить, а
# рушії збирає `nysh htr install`: інтерпретатор і torch — з conda-forge
# (micromamba він приносить сам), решта — pip'ом. Тут лише сказати про це
# заздалегідь, щоб «HTR: немає» в `nysh info` не читалось як поломка.
...
if [ "$(uname -s 2>/dev/null)" = Darwin ] && [ "$(uname -m 2>/dev/null)" = x86_64 ]; then
  case "$SOURCE" in
    *htr*)
      say "ℹ Mac з процесором Intel: PyPI не має колес torch для macOS x86_64, тому"
      say "  рушії читання збираються окремим кроком з conda-forge:"
      say "    nysh htr install"
      say "  Читання йде на процесорі (~2 хв/стор). Решта ставиться як звичайно."
      say ""
      ;;
  esac
fi
