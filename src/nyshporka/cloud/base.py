...
class ChannelDropped(CloudError):
    """Канал до машини обірвався посеред дії. Про саму машину це нічого не каже.

    🔴 Окремий клас, а не голий `EOFError`/`OSError` транспорту: сирий виняток
    летів повз лічильник відмов опитування просто в аварійний обробник, і той
    ВБИВАВ здорову роботу на оплаченій машині через хвилинний обрив зв'язку.
    Той, хто ловить це, перепідключається й повторює — робота на машині живе
    відчеплено, і каналу для цього не потребує.
    """
...
class Need:
    ...
    #: Найстаріша архітектура карти (compute capability), під яку середовище
    #: рушіїв узагалі має колесо torch. `None` — не обмежувати.
    #:
    #: 🔴 Справжня оренда 19.09.2026: бекенд узяв найдешевшу машину ринку — GTX
    #: TITAN X (5.2), — середовище зібралось без жодної помилки, а читання дало
    #: «CUDA error: no kernel image is available» на кожній сторінці. Дешевий
    #: хвіст ринку — це саме старі карти, тож без цієї вимоги оренда стабільно
    #: приводить туди, де читати нема чим.
