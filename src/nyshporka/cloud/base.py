...
class Need:
    ...
    #:
    #: 🔴 3.3, а не 2.5. Шард тримає свої 3.2-3.6 ГБ ЗАВЖДИ — це ваги обох
    #: рушіїв і буфери, і від щільності сторінки вони майже не залежать. Поріг,
    #: нижчий за це, не «сміливий компроміс», а арифметика, яка не сходиться:
    #: вісім шардів по 3.3 це 26 ГБ на 24-гігабайтній карті, і замір дав 62%
    #: сторінок у збоях. Модель «споживання пропорційне стелі рядків»
    #: спростована окремо: найрідші аркуші (75-119 рядків) падали так само.
    #:
    #: 🔴 Але й запас брати не можна: поріг ділить вільну пам'ять ще на ВИБОРІ
    #: машини, тож завищене число мовчки відправляє захід на гіршу карту.
    #: Замір на розворотах: 4.5 дало три шарди й 538 стор/год, 3.3 — сім шардів
    #: і 1750 за ті самі гроші.
    gb_per_shard: float = 3.3
...
class Session(Protocol):
    ...
    #: 🔴 Розкрити шлях у той, що справді існує на машині. Тильда сюди
    #: потрапляє постійно (типова тека роботи — `~/nysh-run`), а до оболонки
    #: вона їде в лапках і не розкривається: замість домашньої теки з'являється
    #: тека з іменем `~`, і робота тихо лягає не туди, куди по неї потім
    #: прийдуть. Розкриває той, хто знає машину, — тобто бекенд.
