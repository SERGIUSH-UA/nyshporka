...
class Convoy:
    ...
    def lines_per_page(self) -> float | None:
        """Щільність для ГРОШЕЙ — максимум по справах.

        🔴 Тут максимум доречний: він розширює вилку бюджету, і захід не
        спиняється на грошах посеред роботи. Мінімум чи середнє дали б зупинку
        на найщільнішій справі — машина відпрацювала, а частина роботи лишилась
        неоплаченою й недочитаною.

        ⚠ Для ВИБОРУ МАШИНИ це число брати НЕ МОЖНА — див.
        `lines_per_page_typical`.
        """
        known = [leg.plan.lines_per_page for leg in self.legs if leg.plan.lines_per_page]
        return max(known) if known else None

    @property
    def lines_per_page_typical(self) -> float | None:
        """Щільність для ЧАСУ Й ВИБОРУ МАШИНИ — середнє, зважене сторінками.

        🔴🔴 Максимум тут коштував трьох порожніх ринків поспіль. Машина читає
        ВСІ сторінки черги, тож час — це сума по сторінках, а не «сторінок ×
        найгустіша». Заміряно 23.09.2026 на черзі 237 справ / 5115 сторінок:
        зважене середнє 38.9 рядків/стор, максимум 251 — і той максимум давали
        ДВІ справи. Прогноз часу через це завищувався в 3.7 раза, а на живому
        ринку зі 142 машин проходило 11 замість 87: відмовляли стелі часу
        («17.9 год > 14.0») і ціни тисячі сторінок.

        Тобто ринок не «став жорстоким» — це ми міряли його по найгустішій
        сторінці всієї черги.
        """
        pages = 0
        total = 0.0
        for leg in self.legs:
            lines = leg.plan.lines_per_page
            if not lines:
                continue
            # Вага — сторінки, які СПРАВДІ читатимуться: у дочитці це
            # `pages_left`, а не весь обсяг справи.
            left = getattr(leg.plan, "pages_left", None)
            n = max(1, int(left if left is not None else (leg.plan.frames or 0)))
            pages += n
            total += n * float(lines)
        return round(total / pages, 1) if pages else None
