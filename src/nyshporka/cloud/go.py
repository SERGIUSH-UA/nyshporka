...
def _prepare(res: GoResult, case: str, say: EventFn, owner: contextlib.ExitStack, *,
    ...
    if plan.base_out is not None:
        ...
        if cache.usable and cache.path is not None:
            ...
            # 🔴 Тут мало стояти не «⚠», а ЦІНА. Сегментація — 74% процесора
            # сторінки, тож без неї захід коштує приблизно вдвічі (замір
            # 18.4 проти 9.1 с/стор). Повідомлення без цього числа читається як
            # дрібниця, і 23.09.2026 саме так і прочиталось: прогін 237 справ
            # поїхав без засіву й мав тривати три години замість півтори.
            #
...
#: Що сказати, коли перечитування не знайшло готової сегментації.
#:
#: 🔴 Число тут головне. Сегментація коштує 74% процесора сторінки
#: (`htr/patches/README.md`), і замір на тому самому матеріалі дає 18.4 с/стор
#: без кешу проти 9.1 з ним. Тобто «сегментації немає» — це не попередження про
#: дрібницю, а подвійний рахунок за весь захід.
_SEG_PRICE = (
    "готової сегментації немає — сторінка коштуватиме ~18.4 с замість ~9.1, "
    "тобто захід приблизно ВДВІЧІ дорожчий і довший. Кеш першого прогону лежить "
    "у чекпоінтах сховища: `gpurunner htr fetch-ckpt --plan <план того прогону>` "
    "принесе його в {where} разом із текстом, і наступний захід поїде із "
    "засівом."
)
