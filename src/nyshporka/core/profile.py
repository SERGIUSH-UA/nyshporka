...
#: Літери, яких у дореформеному російському написанні немає взагалі: побачивши
#: їх, кандидата не існує — «Ліщинськ» це «Лищинск»? «Ліщинск»? Вибір між ними
#: робить документ, а не правило.
_NO_COUNTERPART = set("іїєґ")

#: Літера, яка counterpart має, але не один: «Василенко» лишається «Василенко»,
#: а «Лисенко» стає «Лысенко». Кандидат є, але звірити його треба оком.
_AMBIGUOUS = set("и")


def prereform_hint(stem: str) -> tuple[str, str]:
    """(кандидат дореформеної основи, застереження). Обидва можуть бути порожні.

    🔴 Це ПІДКАЗКА людині, а не запис у файл, і межа тут не в обережності, а в
    даних: перенесення основи між орфографіями заборонене (`core.morph`), бо
    правило «просто заміни літери» помиляється частіше, ніж допомагає. Але є
    вузький клас, де воно не помиляється: основи без літер, яких у тому шарі
    немає — «Василенк» дореформеною пишеться побуквенно так само.

    ⚠ `и` не блокує кандидата, а додає застереження: воно має відповідник, але
    не один. Мовчазна відмова тут коштувала б дорожче за підказку зі
    сумнівом — прізвищ на «-енко» саме з `и` в основі більшість, і людина
    лишилась би без жодного натяку там, де крок очевидний.
    """
    text = (stem or "").strip()
    if not text or _NO_COUNTERPART & set(text.lower()):
        return "", ""
    if _AMBIGUOUS & set(text.lower()):
        return text, ("звірте «и» з документом: у російському написанні на його "
                      "місці буває «ы» (Лисенко → Лысенко)")
    return text, ""
...
def save(name: str, display: str, *, paradigm: str = "adj_skyi",
    ...
    * `updated` — профіль є, поля форми переписуються на МІСЦІ.
    ...
    ⚠ `stems=None` і `stems={}` — РІЗНЕ. Перше означає «про основи не питали»
    (командний рядок знає лише одну орфографію, `--orth`), і наявні лишаються
    недоторканими; друге — «форма надіслала повну картину», і питані шари в ній
    авторитетні, інакше очищене поле не можна було б очистити.
    ...
    **перечитується** і звіряється з наміром. Не зійшлось — відмова з порадою
    ...
    #
    # 🔴 …але рівно доти, доки картину справді надіслали. `stems=None` означає
    # «про основи не питали» — так приходить `nysh profile init`, який знає лише
    # ОДНУ орфографію (`--orth`). Доти обидва випадки виглядали однаково, і
    # другий прогін команди — той самий, який радить попередження про
    # `ru_prereform`, — стирав основу, задану першим: `uk` зникала, число
    # написань падало з 13 до 10, і жодного слова про це не було.
    was_stems = (((_raw().get("profiles") or {}).get(name) or {})
                 .get("surname") or {}).get("stems") or {}
    for k, v in was_stems.items():
        if (stems is None or k not in morph.ORTHOGRAPHIES) and str(v).strip():
...
#
# Поля, яких тут немає, калібруються замірами на власному матеріалі й
# дописуються сюди ж, руками. Пошук читає їх усі:
#
#   confusers   — сусідні прізвища на той самий хвіст. Хіт, який чуже слово
#                 пояснює КРАЩЕ за ваше прізвище, падає вниз видачі з причиною.
#                 Не зникає: пропущений аркуш не відсівається нічим.
#                 Пишуться ВЖЕ нормалізованими (мала літера, щ→sc, ч→c, и/і→i).
#   rank_down   — іменовані регекси службових формул, теж на пониження рангу.
#                 ⚠ теж проти нормалізованого тексту: правило кирилицею не
#                 спрацює ніколи, і пошук про це скаже.
#   kin         — люди роду як ЯКОРІ: {{given, patronymic, born, died}}. Рушій
#                 калічить довге прізвище сильніше за коротке ім'я, тож рід
#                 буває цілий саме в іменах. Роки обов'язкові: ім'я корисне,
#                 поки людина жива, по батькові — поки живі її діти.
#   places      — регіон дослідження: {{name, also, uezd, eparchy, confession}}.
#                 Звідси маршрутизатор джерел (скіл `where-to-dig`) бере повіт
#                 і єпархію, не питаючи щоразу; `also` — історичні назви.
#   archives    — архіви й фонди, де рід уже шукали: {{repo, fond}}.
#   search_forms, synth, anchors, selftest — див. документацію.
fallback: {name}
