...
class Progress:
    ...
    у темпі, ETA на початку прогону хибний у рази. Тому доки темп не набрано,
...
class JobRecord:
    ...
    #: Куди йти далі — структурно, а не переказом у тексті.
    #:
    #: 🔴 Без цього поля довга робота вміла лише сказати, що вона не
    #: остання: «зібране лягло окремим файлом; звести: nysh registry
    #: merge …». Команду терміналу показували тому, хто запустив роботу
    #: з браузера, — тобто попередження називало наступний крок і не
    #: давало жодного способу його зробити.
...
class JobBus:
    ...
    async def forget_finished(self) -> int:
        """Прибрати завершені роботи з переліку. Повертає, скільки прибрано.

        🔴 Журнал переживає перезапуски, тож без цього перелік «Що зараз
        робиться» ставав історією всього, що колись запускали: десяток
        однакових рядків, серед яких треба знайти свій. Активні роботи не
        чіпаються ніколи — прибирається лише те, що вже скінчилось.
        """
        async with self._lock:
            gone = [j.id for j in self._jobs.values() if j.state.final]
            for job_id in gone:
