...
def parse_case_path(rel: str) -> tuple[str, str, str | None, str] | None:
    ...
    if ms:
        ...
        if msh:
            ...
            f1, o1, s1 = _norm_fond(msh.group(1)), _norm_spr(msh.group(2)), msh.group(3)
            if fond and f1 != fond:
                # 🔴 Трійка в імені НЕ є «фонд-опис-справа», якщо фонд уже дав slug
                # і з першою групою він не збігся. Заміряно 2026-09-10 на ДАХмО
                # ф.315: теки `dahmo_315_pages/met1870-315-10037` розбирались як
                # фонд 315 (зі slug'а), ОПИС 315, справа 10037 — 53 записи з
                # фальшивим описом, які аудит ключів читав як колізію описів.
                # А `dahmo_315_small/m904-24-25` (ДАВіО 904-24-25) ставала
                # «ДАХмО 315-24-25» — чужою книгою в ф.315.
                if o1 == fond:
                    spr = _norm_spr(s1)   # «<жанр><рік>-<фонд>-<справа>»: опис невідомий
                else:
                    return _sidecar_case(rel)   # ім'я несе ЧУЖИЙ фонд — не приписувати slug'у
            else:
                fond = fond or f1
                opys = opys or o1
                spr = _norm_spr(s1)
