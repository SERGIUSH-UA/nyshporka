...
_REPO_LABEL = {
    ...
    # 🔴 DACHO (Чернігівська) — не DACHVO (Чернівецька): коди різняться однією
    # літерою, а архіви лежать за 700 км один від одного. Додано 2026-08-18 із
    # першою справою — сповідний розпис Озерян Лохвицького пов. 1782 (ф.1462
    # Прилуцького духовного правління), узятий напряму з FamilySearch.
    "DACHO": "ДАЧО",
    ...
    "KOSTEL": "костел",
    # Решта обласних і центральних архівів України — додано 2026-08-27, щоб
    # шифра з будь-якого архіву (не лише вже опрацьованих) розпізнавалась
    # одразу, а не падала «невідомий архів». Абревіатури звірені по кожній
    # окремій сторінці uk.wikisource.org/wiki/Категорія:Архіви_України, а не
    # вгадані з назви області — там, де дві області ділять перші літери
    # (Дніпропетровська/Донецька, Закарпатська/Запорізька, Харківська/
    # Херсонська, Львівська/Луганська, Київська/Кіровоградська, Севастополь/
    # Сумська), спільнота розрізняє їх додатковою літерою в середині коду, і
    # ця літера підтверджена, а не за аналогією з уже наявними ДАХмО/ДАЧО/ДАЧвО.
    "DAARK": "ДААРК", "DADNO": "ДАДнО", "DADOO": "ДАДоО",
    "DAZKO": "ДАЗкО", "DAZPO": "ДАЗпО", "DAIFO": "ДАІФО",
    "DAK": "ДАК", "DAKO": "ДАКО", "DAKRO": "ДАКрО",
    "DALO": "ДАЛО", "DALUO": "ДАЛуО", "DAMO": "ДАМО",
    "DAPO": "ДАПО", "DARO": "ДАРО", "DAS": "ДАС", "DASO": "ДАСО",
    "DATO": "ДАТО", "DAHO": "ДАХО", "DAHEO": "ДАХеО",
    # ДАЧкО (Черкаська) — бракувало саме її: сусідні ДАЧО (Чернігівська) і
    # ДАЧвО (Чернівецька) вже були, Черкаська — ні, хоча в цьому дослідженні
    # саме до неї відносяться справи Мутихів (Золотоніський пов.).
    "DACHKO": "ДАЧкО",
    # Центральні історичні — не обласні, але так само справи без них
    # не розпізнавались; ЦДІАК (Київ) уже був, Львів і два центральні
    # додано поруч.
    "CDIAL": "ЦДІАЛ", "CDAVO": "ЦДАВО", "CDAHOU": "ЦДАГОУ",
}
...
_REPO_ALIAS = {
    ...
    "АНРМ": "ANRM", "ДАЧВО": "DACHVO", "ДАЧО": "DACHO",
    "ДАХМЕЛЬНО": "DAHMO",
    "ДААРК": "DAARK", "ДАДНО": "DADNO", "ДАДОО": "DADOO",
    "ДАЗКО": "DAZKO", "ДАЗПО": "DAZPO", "ДАІФО": "DAIFO",
    "ДАК": "DAK", "ДАКО": "DAKO", "ДАКРО": "DAKRO",
    "ДАЛО": "DALO", "ДАЛУО": "DALUO", "ДАМО": "DAMO",
    "ДАПО": "DAPO", "ДАРО": "DARO", "ДАС": "DAS", "ДАСО": "DASO",
    "ДАТО": "DATO", "ДАХО": "DAHO", "ДАХЕО": "DAHEO",
    "ДАЧКО": "DACHKO",
    "ЦДІАЛ": "CDIAL", "ЦДАВО": "CDAVO", "ЦДАГОУ": "CDAHOU",
}
