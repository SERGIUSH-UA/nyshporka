...
Четвертий різновид, `web`, у цьому репозиторії не генерує нічого: його
забирає функцією сервер Супряги, який ставить пакет піном на коміт.
...
def render_web(brand: Brand) -> str:
    """Токени порталу: ті самі кольори плюс веб-типографіка.

    🔴 Різновиду немає в `targets()` навмисно — у ЦЬОМУ репозиторії він нічого
    не генерує. Його споживач — сервер Супряги: він ставить пакет піном на
    коміт, кличе цю функцію й кладе результат у свою статику. Через це копії
    палітри не існує ніде, а саме копія колись і розійшлася (`#7a4f2b`,
    переписаний руками в CSS).

    🔴 Шрифти виходять ПЕРЕВИЗНАЧЕННЯМ тих самих імен (`--font-text`,
    ...
    розгортання, і зашитий сюди він зробив би джерело бренду залежним від
    чужої теки.

    Порожній `type.web` не є помилкою: блок тоді не виводиться, і сайт
    ...
    pairs = [
        ...
        ("line-height", brand.web_leading, "інтерліньяж сайту"),
    ]
    live = [(css, value, why) for css, value, why in pairs if value]
    if not live:
        return render_app(brand)

    width = max(len(f"  --{css}: {value};") for css, value, _ in live)
    rows = [f"  --{css}: {value};".ljust(width) + f"   /* {why} */\n"
            for css, value, why in live]
    return "".join([
        render_app(brand),
        "\n" + _section(
