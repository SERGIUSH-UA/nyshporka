...
const STRINGS = {
  uk: {
    ...
    'fonds.none.why': 'Це НЕ означає, що фондів немає — означає, що сказати '
      + '«справи в архіві не існує» поки нема на чому. Зберіть опис нижче або '
      + 'командою `nysh registry collect`, потім `nysh registry merge`.',
    ...
    'fonds.collect.why': 'Обхід джерела кладе свій файл окремо; реєстр '
      + 'з’являється аж після злиття. Тому кроків два, і другий не '
      + 'необов’язковий.',
    ...
    'fonds.collect.plan': 'Порахувати',
    'fonds.collect.go': 'Зібрати',
    ...
    'sources.why': 'ЗОВНІШНІ довідники: сайти архівів, покажчики плівок, '
      + 'Вікісховище. Не плутати з описами фондів — там наш власний реєстр, і '
      + '«немає» означає «в архіві не існує».',
    'sources.q': 'Село, прізвище або слово із заголовка справи',
    'sources.where': 'Де саме шукаємо',
    'sources.count': 'шукати є на чому в {ok} із {n} джерел, що вміють шукати '
      + '(усього джерел {all})',
    'sources.nosearch': 'пошуку не має — лише перегляд і завантаження',
    'sources.blind': 'шукати нема на чому: обходу немає',
    ...
    'sources.ceiling': 'показано {n} — це СТЕЛЯ ЗАПИТУ, а не число знахідок. '
    ...
    'intake.why': 'Матеріал на диску, який ще нічим не є: без шифри його не '
      + 'занести в облік і не послатись на знахідку. Опишіть — і справа '
      + 'з’явиться в бібліотеці.',
  ...
  en: {
    ...
    'fonds.none.why': 'This does NOT mean there are no fonds — it means there '
      + 'is nothing on which to say “no such case exists in the archive”. '
      + 'Collect one below, or run `nysh registry collect`, then `merge`.',
    'fonds.act.lib': 'this case in the library',
    'fonds.act.take': 'take it: download and register',
    'fonds.collect': 'Collect a fond inventory',
    'fonds.collect.why': 'A crawl drops its own file aside; the finding aid '
      + 'appears only after the merge. Hence two steps, and the second is not '
      + 'optional.',
    ...
    'fonds.collect.plan': 'Estimate',
    'fonds.collect.go': 'Collect',
    ...
    'sources.why': 'EXTERNAL reference sets: archive sites, film indexes, '
      + 'Wikimedia Commons. Not to be confused with fond inventories — those '
      + 'are our own, and “none” there means “does not exist in the archive”.',
    'sources.q': 'Village, surname, or a word from the case title',
    'sources.where': 'Where exactly we look',
    'sources.count': 'there is something to search in {ok} of {n} sources that '
      + 'can search (sources in total: {all})',
    'sources.nosearch': 'no search — browsing and fetching only',
    'sources.blind': 'nothing to search: no crawl collected',
    ...
    'sources.ceiling': 'showing {n} — that is the QUERY CEILING, not the number '
      + 'of matches. Narrow the query or pick one source.',
