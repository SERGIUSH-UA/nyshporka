...
// вибору шляху, а екрани цю кнопку вже малюють. Порядок тут значення не має
// (реєстри наповнюються на завантаженні), але рядок стоїть явно з тієї самої
// причини, що й екрани нижче: без нього кнопка є, а натискати нічого.
...
async function boot() {
  ...
  // екрані те саме. Адресний рядок при цьому починав описувати не той
  // екран, що показано, і за ним ішли гортання сторінок (`data-act=page`)
  // та гарячі клавіші — обидва читають хеш.
  //
  // ⚠ Порівняння з `currentScreen()` — це захист від рекурсії: власний
  // запис `show()` у хеш теж збуджує подію, і без нього кожен перехід
  // викликав би себе вдруге.
  //
  // ⚠ Посів (`ST[screen]`, див. `goto`) в адресу не потрапляє, тож екран,
