 ...
 *
 * ⚠ Цілий знімок важить близько мегабайта проти 15 КБ на вирізку рядка. Тут
 * він однаково показується одразу — сюди приходять саме дивитись на аркуш, —
 * але його можна сховати, і тоді крок сторінки перестає коштувати мережі.
 */
...
let VS = {
  run: '', meta: {}, pages: [], i: -1,
  lines: [], geo: {}, zoom: 100, alt: null,
  // 🔴 Знімок показується ОДРАЗУ. Спершу він відкривався окремою дією — щоб
  // не платити мегабайтом там, де досить рядка, — але в гортачі це рахунок не
  // з того боку: сюди приходять саме ДИВИТИСЬ на аркуш, і місце під нього
  // розмітка тримає в будь-якому разі. Порожня половина екрана поруч із
  // текстом читається як «сторінка не завантажилась», а не як ощадність.
  //
  // Вимкнути можна тією ж кнопкою — тоді лишається сам текст, і крок сторінки
  // перестає коштувати мережі.
  shot: true,
};
...
async function viewOpenRun(run, page = '', line = null) {
  ...
  const d = env.data || {};
  // ⚠ `shot` навмисно НЕ скидається: це вибір людини — дивитись на знімок чи
  // читати самий текст, — і він мусить пережити перехід на іншу справу.
  // Скидаючи його тут, ми щоразу відкривали б знімок наново тому, хто його
  // щойно сховав.
...
async function viewShow(i, line = null) {
  ...
  // Рамки приїхали — тепер знімок разом з оверлеєм. Порядок навмисний: текст
  // уже на екрані, тож людина читає, поки їде зображення.
