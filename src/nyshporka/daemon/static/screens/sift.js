...
Object.assign(ACTIONS, {
  ...
  'sift.note': () => {
    const h = ST.sift.hits[ST.sift.i] || {};
    // 🔴 Ключ справи, а не ім'я теки прогону. Доти, доки хіт ключа не ніс,
    // сюди летіло `h.name` — і облік відповідав нормативною відмовою «не
    // розпізнав справу «sample-315-159»», хоч підставила це значення сама
    // морда. Ланка «Пошук → Розбір → Око» не працювала жодного разу.
    const case_ = h.case_key || h.shifra || '';
    ST.eye = { ...(ST.eye || {}), case: case_, scan: h.page };
    return show('eye');
  },
});
