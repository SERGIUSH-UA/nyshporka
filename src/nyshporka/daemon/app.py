...
#: Імена, під якими застосунок відповідає завжди — незалежно від того, яку
#: адресу передали в `--host`. Ім'я, з яким його реально підняли (LAN-адреса
#: чи `0.0.0.0`), додається до цього набору нижче, в `create_app`: перевірка
#: й тоді лишається виключним переліком, а не відкритим правилом «що завгодно
#: не з інтернету» — чужий сайт, що перев'язав своє ім'я на нашу адресу,
#: захисту так само не пройде.
_OWN_NAMES = frozenset({"127.0.0.1", "localhost", "::1", "0.0.0.0"})
...
def create_app(ws: Workspace | None = None, *, token: str = "",
    ...
    `host` — адреса, на якій демон реально піднято (`serve(host=...)`);
    додається до переліку довірених імен поряд із петлею, щоб запит,
    адресований на цю ж адресу, не відбивало як чужий. `None` (дефолт) — та
    сама, лише петльова, поведінка, що й раніше.
    """
    try:
    ...
    # Адреса, на якій демон реально підняли, — довірена нарівні з петлею.
    # Перелік лишається виключним: усе, що не в ньому, і далі відбивається.
    trusted_names = _OWN_NAMES | ({host.strip().lower()} if host else set())
    ...
    async def only_this_machine(request, call_next):  # type: ignore[no-untyped-def]
        ...
        🔴 Захист від перев'язування імені, не від мережі як такої. Сайт із
        коротким TTL віддає спершу свою адресу, а через секунду — довірену
        (петлю чи ту, на якій підняли `--host`); для браузера це лишається той
        самий origin, тож ані CORS, ані `SameSite` не спрацьовують, і його
        скрипт починає говорити з нашим демоном як своя ж сторінка.
        ...
        Тунель (`ssh -L`) шле `localhost` і проходить завжди; LAN-адреса
        проходить, лише якщо демон свідомо піднято на ній (`serve(host=...)`)
        — тоді вона в `trusted_names`, інакше ні.
        """
        raw_host = request.headers.get("host") or ""
        req_host = raw_host.rsplit(":", 1)[0].strip("[]").lower()
        if req_host and req_host not in trusted_names:
            return JSONResponse(
                {"ok": False, "v": 1, "data": {}, "warnings": [],
                 "error": f"застосунок відповідає лише на {', '.join(sorted(trusted_names))}, "
                          f"а запит прийшов з іменем «{req_host}»"},
                status_code=403)
...
def serve(host: str = DEFAULT_HOST, port: int = DEFAULT_PORT, *,
    ...
    Дефолт — петля. `host` іншим, ніж петля, робить лише той, хто явно
    попросив про це (`nysh serve --host`, не мовчазний дефолт): це застосунок
    з архівом однієї людини, канонічні дані про живих родичів, сканами й
    нотатками, тож CLI друкує застереження щоразу, коли `host` не петльовий —
    щоб «на хвилинку» не лишалось непоміченим.
    """
    try:
        import uvicorn
    except ImportError as exc:  # pragma: no cover — extras `app`
