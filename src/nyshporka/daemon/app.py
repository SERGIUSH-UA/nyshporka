...
#: Імена, під якими застосунок погоджується відповідати. Перелік вичерпний і
#: розширенню не підлягає: усе, що не петля, — це або чужий сайт, який
#: перев'язав своє ім'я на нашу адресу, або спроба виставити архів однієї
#: людини назовні. Обидва випадки лікуються однаково.
...
def create_app(ws: Workspace | None = None, *, token: str = "") -> FastAPI:
    ...
    async def only_this_machine(request, call_next):  # type: ignore[no-untyped-def]
        ...
        🔴 Це не параноя про мережу: бінд і так лише на петлю. Це захист від
        перев'язування імені. Сайт із коротким TTL віддає спершу свою адресу, а
        через секунду — `127.0.0.1`; для браузера це лишається той самий origin,
        тож ані CORS, ані `SameSite` не спрацьовують, і його скрипт починає
        говорити з нашим демоном як своя ж сторінка.
