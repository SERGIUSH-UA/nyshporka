"""🤖 MCP-сервер: підмножина реєстру операцій, згенерована з нього ж.

🔴 Не «агентна поверхня» — саме підмножина. Повна поверхня в агента є й без
MCP: `nysh op <ім'я>` дістає будь-яку операцію, `--describe` віддає схему й
докстрінг. Сервер існує там, де перелік tool'ів дешевший за виклик команди, і
платить за це стелею (нижче), тобто показує менше половини реєстру. Плутати
його з «усім, що вміє застосунок» — прямий шлях до висновку «Нишпорка цього не
може» там, де вона може.
...
def serve() -> int:
    ...
    # 🐾 Агент бачить Нишпорку тут — і мусить бачити те саме, що людина:
    # ім'я, лінію бренду й три правила, від яких залежить, як читати відповіді.
    # Це не оформлення: `instructions` заходять у контекст кожної сесії, тож
    # довший текст коштував би токенів на кожному виклику, а коротший лишив би
    # агента без того, чим тут міряють результат. Решта — в `AGENTS.md`.
    ...
    server = Server(
        ...
        version=__version__,
        instructions="\n".join((
            # Першим рядком: інструкції — єдине, що агент із підключеним MCP
            # читає в кожній сесії.
            "⚠ MCP-сервер Нишпорки застарів і зникне в 0.19 — користуйся `nysh` "
            "у терміналі (`nysh op <ім'я>`): там усі операції, зокрема віддача "
            "в Супрягу.",
            f"{b.name_uk} ({b.name_en}) — {b.line_uk}",
            "Читання рукописних архівних справ і пошук прізвища в них.",
            "",
            "🔴 Нуль без знаменника — не результат: перед словом «немає» назви, "
            "чи читабельний декод, скільки сторінок прошукано і чи бачить "
            "конвеєр відомий позитив у цій самій справі.",
            "🔴 Виявити ≠ перевірити: подавай кандидата з вирізкою рядка — "
            "вирішує око дослідника, а не другий рушій.",
            "🔴 Цифра з декоду — не факт: суми, площі, кількість душ і дати "
            "звіряй із зображенням.",
            "",
            "Рушії читання: [П] Писар · [Д] Дяк · [С] Скриба.",
        )),
    )
