...
def _viewer_id(text: str) -> str:
    """Номер справи з адреси переглядача — або порожньо.

    Приймає і повну адресу (`https://…/file-viewer/51068/`), і сам хвіст:
    людина копіює з рядка браузера, а не переписує число.
    """
    m = re.search(r"/file-viewer/(\d+)", text) or re.search(r"^\s*(\d+)\s*$", text)
    return m.group(1) if m else ""


def _resolve_fond_id(given: str, collector: ArchiumCollector, target: Any) -> str:
    """Що дала людина: номер фонду чи посилання на справу.

    🔴 Калібрування з ОДНІЄЇ справи. Знати, що таке «внутрішній номер фонду»,
    людина не мусить — вона вміє показати пальцем: ось справа цього фонду.
    Сторінка переглядача сама несе посилання на свій фонд, тобто номер дає САЙТ,
    а не наш здогад; далі він лягає в сайдкар і більше не питається.

    ⚠ Голе число — це НОМЕР ФОНДУ, а не справи: обидва числові, і вгадувати тут
    не можна. Ознакою справи є саме адреса.
    """
    v = (given or "").strip()
    if not v:
        return ""
    if v.isdigit():
        return v                      # людина дала номер фонду як є
    case_id = _viewer_id(v)
    if not case_id:
        return v                      # хай збирач сам скаже, що це не адреса
    return collector.fond_id_from_case(case_id, target.repo)
...
class ArchiumCollector:
    ...
    def known_fond_id(self, target: Target) -> tuple[str, dict[str, str]]:
        ...
        # 🔴 Далі — пак архівів. Номер, знайдений одним користувачем, однаковий
        # для всіх: це факт про сайт, а не про його простір. Доти кожен мусив
        # шукати те саме число заново, а без нього збирач узагалі не стартував.
        #
        # ⚠ Сайдкар сильніший навмисно: він від ЦІЄЇ машини й від сьогодні, а
        # пак їде з версією пакета й може відстати від перебудованого сайту.
        from nyshporka.archives import active

        f = active().fonds.get((target.repo.upper(), str(target.fond)))
        if f is not None and f.archium_fond:
            return f.archium_fond, dict(f.archium_opys or {})
        return "", {}
    ...
    def plan(self, target: Target) -> Plan:
        ...
        if not fond_id:
            return Plan(
                ...
                needs={"fond_id": "номер фонду на сайті або посилання на "
                                  "будь-яку його справу"},
                why=("сайт адресує фонд власним номером, і з архівним він не "
                     "пов'язаний (ф.224 значиться фондом 198). Найпростіше — "
                     "дати посилання на БУДЬ-ЯКУ справу цього фонду в "
                     "переглядачі: номер візьметься зі сторінки сам і більше "
                     "не питатиметься. Відомий номер приймається як є."))
    ...
    def collect(self, target: Target, *, dest: Path,
        ...
        fid = _resolve_fond_id(fond_id, self, target) or known_id
        if not fid:
