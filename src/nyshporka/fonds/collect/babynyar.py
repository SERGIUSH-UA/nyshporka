...
def years(dates: str, title: str = "") -> tuple[str, str]:
    """«1931 - 1932» або «16.01.1853 - 16.06.1854» → (рік від, рік до).

    ⚠ Береться саме перший і ОСТАННІЙ чотиризначник рядка: у датах із днями
    чисел більше двох, і наївне «перші два» дало б 16.01 як рік.
    ...
    колонці мали 93 (6%). Реєстр без років за роками не фільтрується, тобто
    втрачає те, заради чого його й збирають.

    ⚠ Із заголовка береться найменший і найбільший рік, а не перший і
    останній: там вони перелічені по типах запису й ідуть не в порядку
    зростання.
    """
    found = _YEAR.findall(dates or "")
    if found:
        return found[0], found[-1]
    # 🔴 У заголовку чотиризначник буває й не роком: «Справа 1205. … за 1932 р.»
    # або «12345 арк.» давали рік 1205 чи 1234, і реєстр фільтрувався б за
    # вигаданою датою. Тут рік — лише ціле число з правдоподібного діапазону.
    in_title = [int(y) for y in _TITLE_YEAR.findall(title or "")]
    if not in_title:
        return "", ""
    return str(min(in_title)), str(max(in_title))
        ...
        for i, (did, opys) in enumerate(picked, 1):
            ...
            for cid, c in table_rows(src.page(f"/archive/desc/{did}"), "case"):
                ...
                if not scans or scans == "0":
                    offline += 1
                y1, y2 = years(c[2] if len(c) > 2 else "",
                               c[1] if len(c) > 1 else "")
                rows.append({
