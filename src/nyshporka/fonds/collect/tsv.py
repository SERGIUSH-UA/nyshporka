...
#: ⚠ Групу справ одним рядком («29-35») цей розбір зводить до ПЕРШОГО номера —
#: так і треба, бо ключ групи саме він. Кінець групи дає `group_end`.
...
#: Група справ одним рядком опису: «29-35», «Справа 3548-550».
_GROUP = re.compile(r"^(\d+)\s*[-–—]\s*(\d+)(?!\d)")


def group_end(raw: str) -> str:
    """Кінець групи справ одним рядком: «29-35» → "35"; не група — "".

    📚 Описи бібліотек і частини архівів записують кілька справ одним рядком з
    аркушами на всю групу. Рядок лишається один, з ключем за першим номером
    (його дає `split_code`), а кінець іде в колонку `spr_to` — інакше покриття
    звітує решту номерів групи відсутніми.

    🔴 Кінець набирають скорочено: «3548-550» — це 3548–3550. Без розгортання він
    виходив меншим за початок, і група мовчки ставала однією справою.

    ⚠ Скорочення розгортається лише з двох і більше цифр кінця: «10-9» чи
    «12-3» — це сміття набору, а не 10–19 і 12–13, і вигадана група завищила б
    покриття.

    ⚠ Кінець, що й після розгортання не більший за початок, групою не є: рядок
    лишається справою з першим номером, без кінця.
    """
    m = _GROUP.match(_NUM_PREFIX.sub("", raw or "").strip())
    if not m:
        return ""
    a_s, b_s = m.group(1), m.group(2)
    a, b = int(a_s), int(b_s)
    if b <= a and 2 <= len(b_s) < len(a_s):
        b = int(a_s[:len(a_s) - len(b_s)] + b_s)
    return str(b) if b > a else ""
