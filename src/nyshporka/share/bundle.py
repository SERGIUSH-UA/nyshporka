...
#:
#: 1 — геометрія лежала ВСЕРЕДИНІ текстового пакета, і `Voice.geometry`
#:     означав «пакувальник поклав її туди».
#: 2 — геометрія їде окремим файлом (`GEOM_SUFFIX`), а `Voice.geometry`
#:     означає «рамки є на диску пакувальника», тобто «сервер має видати
#:     посилання на другий об'єкт».
#:
#: 🔴 Число росте з КОЖНОЮ зміною розкладки, доки формат не заморожено. Два
#: різні пакети під одним номером роблять поле версії марним рівно тоді, коли
#: воно потрібне: читач не може відмовити явно, бо йому нічим відрізнити, а
#: «те саме поле означає інше» — це розбіжність, якої не видно ніде.
#: Заморозиться число перед запуском пулу; доти піднімати його коштує рядок,
#: а не розбір, чий `schema: 1` який.
SCHEMA = 2
