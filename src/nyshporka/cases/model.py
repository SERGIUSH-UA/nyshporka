...
class CaseRow:
    ...
    # ── обсяг прочитаного (з `_htr_meta.json`: `chars`/`lines` на сторінку) ──
    # 🔴 Два різні числа, і плутати їх не можна. `*_max` описує МАТЕРІАЛ: скільки
    # тексту дала справа, рахуючи кожен кадр один раз — по найповнішому голосу.
    # `*_all` описує РОБОТУ рушіїв: та сама сторінка, прочитана Писарем і Дяком,
    # входить двічі. Вони розходяться майже вдвічі (448 424 сторінко-декоди на
    # 253 тисячі кадрів), тож підсумок з однією цифрою відповідає лише на одне з
    # двох питань і мовчки видає його за друге.
    htr_pysar_chars: int = 0
    htr_diak_chars: int = 0
    htr_skryba_chars: int = 0
    htr_pages_all: int = 0     # сторінко-декоди: кадр × голос
    htr_chars_all: int = 0
    htr_chars_max: int = 0     # символів у найповнішому голосі
    htr_lines_max: int = 0
    htr_pages_blank: int = 0   # сторінок, де найповніший голос не дав НІЧОГО
