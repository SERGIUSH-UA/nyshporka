    ...
    for attempt in range(1, args.supervise + 2):
        ...
        if not gone:
            ...
            if rc == 0 or not pages_all or attempt > args.supervise:
                break
            # 🔴🔴 Під клеймами «неповно» (rc=3) без жодної ДОСТУПНОЇ пропущеної
            # сторінки означає одне: решту зараз читають ЖИВІ сусіди. Це не
            # відмова цього шарда, і повторний воркер тут лише платить за моделі:
            # 23.09.2026 кожен шард на кожній справі черги піднімав другого
            # воркера (7 с моделей + прохід по 15 зайнятих сторінках, «0
            # розпізнано · 15 скіп») — ~10–15 с на справу при читанні ~15 с.
            # Сторінку, чий клейм лишив померлий воркер, `claimable_missing`
            # бачить як доступну, тож справжня смерть сюди не потрапляє.
            if claim_mode and rc == 3:
                rc = 0
                break
...
def main() -> int:
    ...
    ap.add_argument("--seg-height", type=int, default=0,
                    help="висота ресайзу сторінки для сегментера (0 = рідна "
                         ...
                         "8 сторінок медіанної щільності (944 рядки), розкид "
                         "між повторами 0.1%%: 1440 дає 1.19x, 1200 — 1.35x "
                         "(а не 1.35x/1.7x, як стояло тут раніше). Ціна: фузі-"
                         "збіг довгих слів із великої (носіїв прізвищ) 90.6%% "
                         "при 1440 і 90.1%% при 1200. Тобто втрата та сама, а "
                         "виграш удвічі більший — між цими двома брати 1200. "
                         "⚠ Лічильник символів тут БРЕШЕ: при 1440 їх менше "
                         "лише на 0.8%%, а точний збіг слів — 71%%; нижча "
                         "висота не обрізає текст, вона його ПЕРЕПИСУЄ")
...
def _main_case(args: argparse.Namespace, cache: dict | None = None) -> int:
    ...
    meta: dict = {"version": 1, "case_dir": str(case_dir).replace("\\", "/"),
                  ...
                  "case_key": resolve_case_key(case_dir, out_dir.name,
                                               args.case_key),
                  # 🔴 Скільки кадрів мала справа — знаменник, а не лічильник
                  # зробленого. Без нього гортач не може довести відповідність
                  # «кадр → сторінка PDF» для часткового прогону (обірваного,
                  # точкового, шардованого з утратою сторінки): доказ будується
                  # на щільності 1..N, а прочитаних кадрів менше за сторінки в
                  # PDF. Через це аркуш не показувався навіть тоді, коли PDF
    ...
    # ⚠ білий список, а не «усе, крім pages»: поле, якого тут немає, тихо
    # зникає при шардингу — бо зведення бере шапку з part-файла. Саме так
    # одного разу вже загубились плівки FS у реєстрі опису. `frames_total`
    # мусить бути тут: без нього гортач на шардованому прогоні втрачав
    # знаменник і відмовлявся показувати аркуш.
