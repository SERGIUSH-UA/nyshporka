...
def shell_arg(value: str) -> str:
    """Аргумент для оболонки: у лапках, якщо в ньому є пробіл чи дужка.

    🔴 Не косметика. Імена прогонів беруться з назв тек, а ті в людей виглядають
    як «ДАКрО 705-1-1 (Н-Арх - Варваринська церква 1882-1884)» — з пробілами й
    дужками. Підказка, зліплена без лапок, розсипається на десяток аргументів у
    будь-якій оболонці, тобто скопійована команда падає, і людина читає це як
    «порада не працює» (звіт користувача 29.08.2026).
    ⚠ Подвійні лапки, а не одинарні: в іменах трапляється апостроф («Мар'янівка»),
    і одинарні лапки на ньому б і зламались. Символу «"» у назві теки не буває —
    Windows його не дозволяє, а POSIX-теки з ним у цей шлях не потрапляють.
    """
    s = str(value or "")
    if s and not (s != s.strip() or any(c in s for c in " 	()[]&|;<>$`\"\\")):
        return s
    # ⚠ Усередині подвійних лапок POSIX-оболонка ВСЕ ОДНО розкриває `$` і
    # зворотну лапку — тобто тека з іменем `спр-100$HOME` дала б підказку, яка
    # тихо підставить чуже значення замість того, щоб хоча б чесно впасти. Тому
    # ці символи екрануються, а не просто загортаються.
    for ch in ("\\", '"', "$", "`"):
        s = s.replace(ch, "\\" + ch)
    return f'"{s}"' 
...
def _page_image(run: str, page: str) -> Any:
    ...
    if not tried:
        raise ViewError(
            ...
            f"Прив'яжіть прогін до справи — кнопка стоїть тут-таки й на екрані "
            f"«Прогони», у блоці нічиїх. Те саме командою: "
            f"nysh cases bind {shell_arg(run)} <ключ>")
    raise ViewError(
