...
def stamp_of(run: str) -> str:
    ...
    🔴 РОЗМІР мети — третя складова, і саме вона тут працює. `st_mtime_ns`
    називається в наносекундах, але роздільність дає файлова система: заміряно
    на NTFS — **0.34 мс**. У 300 повторах «дописали сторінку й оновили мету»
    mtime мети не змінювався 161 раз, mtime теки 261 раз, а весь штамп із двох
    часів — 161 раз, тобто в половині випадків. Тобто справу дочитали, а пошук
    цього не бачив — рівно та брехня, проти якої штамп і вигадали.
    Розмір безкоштовний (той самий `stat`) і від годинника не залежить: раннер
    дописує сторінку в `pages`, і мета РОСТЕ.
...
# 🔴 Перевірено заміром, тричі, і всі три способи розпаралелити відпали:
#
# * потоки — ×0.9 на чотирьох, ×0.6 на восьми: rapidfuzz тримає GIL увесь свіп;
# * `cdist(workers=-1)` — ×0.9: він паралелить по запитах, а запит тут один;
# * пул процесів — 435 с проти 216 на повному корпусі. На Windows робітник
#   піднімається повторним імпортом пакета, і десять таких імпортів коштують
#   більше за саму роботу. Гірше: викликач без `if __name__ == "__main__"`
#   отримує від такого пулу власний скрипт у кожному робітнику — тобто множення
#   роботи, яке виглядає як зависання.
#
# Тому прискорення шукали не в ядрах, а в тому, щоб не робити зайвого: індекс
# на диску замість перебудови щоразу (13 хв → 0), удвічі дешевша нормалізація,
# межі рядків замість власника на кожного кандидата.
