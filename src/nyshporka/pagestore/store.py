...
# ДАВіО/ДАВО ф.Р-6129 і ДАХмО ф.Р-100 — радянські фонди з літерним префіксом
# («Р-…», окрема нумерація), і `cases.list` друкує їхній ключ саме так
# (`DAHMO/R-100/7`, `_mk_key`), а цей регекс до issue #21 такий рядок назад не
# приймав: фонд без опису в ключі відмовляв, тоді як той самий фонд ІЗ описом
# («DAVIO/R-6129-24/5») рятував інший канал (`_ADDR_RE`, нижче в ланцюжку).
# ⚠ Друга гілка альтернативи — буквально старий вираз, незайманий: коли фонд
# літерний І несе дефіс-опис в одному сегменті («R-6129-24»), `FOND_TOKEN` сам
# забирає лише «R-6129», відкат на другу гілку теж не бере літери — весь збіг
# провалюється, і рядок, як і раніше, іде рятуватись у `_ADDR_RE`. Розрізнити
# «де в „R-6129-24” межа фонду і опису» без цього провалу — довелось би або
# гадати, або дублювати ту саму відповідь, яку вже дає `_ADDR_RE`.
# ⚠ `re.IGNORECASE`: шифра (`_SHIFRA_RE` вище по файлу) і `library.parse_address`
# уже приймають фонд з малої літери («р-100»), а цей ключ — до цієї правки не
# приймав. Ризик малий: репо (група 1) і клас справи (група 3) в обох регістрах
# приймались і раніше (`dahmo/315/8433` уже працював), а `FOND_TOKEN` без цього
# прапорця бере лише ВЕЛИКИЙ літерний префікс.
_KEY_RE = re.compile(rf"^([A-Za-z]+)/((?:{FOND_TOKEN})|(?:\d+(?:-\d+)?))/([0-9A-Za-z@_]+)$",
                     re.IGNORECASE)
...
_ACCEPTED_FORMATS = (
    ...
    "ключ без опису «DAHMO/315/8433» або «DAHMO/R-100/7»; "
    "source-id «S_<архів>_F<фонд>_D<справа>»; "
...
def resolve_case(value: str) -> CaseRef:
    ...
    if m:
        ...
        # ⚠ Фонд — саме через `_norm_fond`, не `_norm_spr`: остання лише знімає
        # провідні нулі й ЛОВЕРКЕЙСИТЬ рядок, тобто «R-100» стало б «r-100» —
        # і бібліотека, де фонд лежить канонічним «R-100» (`_norm_fond` скрізь
        # інде), більше цей запис не знаходила б.
